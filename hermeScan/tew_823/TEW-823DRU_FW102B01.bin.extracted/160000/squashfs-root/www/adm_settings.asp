...
function restoreConfirm(){
		...
		if(confirm(get_words('up_rb_4')+"\n"+get_words('up_rb_5'))){	
				document.form2.html_response_message.value=get_words('_restore');
		    	send_submit("form2");
	   	}
}
...
function confirm_reboot(){
		if(confirm(get_words('up_rb_1')+"\n"+get_words('up_rb_2'))){	
			document.form6.html_response_message.value=get_words('_reboot_running');
			send_submit("form6");
		}
}
...
<div class="box_tn">
	...
	<form id="form1" name="form1" method="post" action="upload_configure.cgi" enctype="multipart/form-data">
		...
		<td colspan="2" class="btn_field">
			<input type="button" class="button_submit" id="load" value="" name="load" onclick="loadConfirm()" />
			<script>$('#load').val(get_words('_import'));</script>
			<input type="reset" class="button_submit" id="set_cancel" value="Cancel" onclick="window.location.reload();" />
...
<div class="box_tn">
	...
	<form id="form2" name="form2" method="post" action="apply.cgi">
		<input type="hidden" name="html_response_return_page" value="adm_settings.asp">
		<input type="hidden" name="html_response_page" value="count_down.asp">
		<input type="hidden" id="html_response_message" name="html_response_message" value="">
		<script>get_by_id("html_response_message").value = get_words('_restore');</script>
	...
	<table cellspacing="0" cellpadding="0" class="formarea">
	<tr>
		<td class="CL"><script>show_words('_qos_txt08');</script></td>
		<td class="CR">
			<input type="button" class="button_inbox" id="restore" name="restore" value="Load Default" onclick="restoreConfirm();" />
		</td>
...
<div class="box_tn">
	...
	<form id="form6" name="form6" method="post" action="reboot.cgi">
		...
		<input type="hidden" name="html_response_return_page" value="adm_settings.asp">
		<input type="hidden" id="html_response_message" name="html_response_message" value="">
		<script>get_by_id("html_response_message").value = get_words('_reboot_running');</script>
		<input type="hidden" name="result_timer" value="50">
		...
		<tr>
			...
			<td class="CR">
				<input type="button" class="button_inbox" value="Reboot" id="restart" name="restart" onclick="confirm_reboot();" />
			</td>
