...
<title>TRENDNET | modelName | Wireless 2.4GHz | Advanced</title>
...
<script type="text/javascript">
	...
	var lanCfg = {
		'enable':        ['<!--# echo wlan0_vap0_enable -->'],	//gz_enable  ,
		'channel':       ['<!--# echo wlan0_channel -->'],
		'exchannel':     [1],
		'coexi':         ['<!--# echo wlan0_disablecoext -->'],
		'chanwidth':     ['<!--# echo wlan0_channel_bandwith -->'],
		'standard':      ['<!--# echo wlan0_dot11_mode -->']
	}	

	var advCfg = {
		'bgprotection':  '1',
		'beaconperiod':  ['<!--# echo wlan0_beacon_interval -->'],
		'dtim':          ['<!--# echo wlan0_dtim -->'],
		'fragthres':     ['<!--# echo wlan0_fragmentation -->'],
		'rtsthres':      ['<!--# echo wlan0_rts_threshold -->'],
		'txpower':       ['<!--# echo wlan0_txpower -->'],
		'spreamble':     ['<!--# echo wlan0_short_preamble -->'],
		'txburst':       ['<!--# echo wlan0_burst_enable -->'],
		'pktaggregate':  [1],
		'ieee11h':       [1]
	}
	
	var qosCfg = {
		'wmmenable':     ['<!--# echo wlan0_wmm_enable -->'],
		'apsd':          [1],
		'dls':           [1]
	}
	var mediaCfg = {
	...
	var m2uCfg = {
		'm2u':           ['<!--# echo wlan0_multicast_stream_enable -->']
	}
	...
	var htCfg = {
		'operating':     [1],
		'shortgi':       ['<!--# echo wlan0_short_gi -->'],
		'rdg':           [1],
		'amsdu':         [1],
		'autoba':        [1],
		'declineba':     [1],
		'ampdu':         ['<!--# echo wlan0_enable_ampdu -->']
	}
	...
	function check_value()
	{
		...
		** Date:	2013-03-18
		** Author:	Moa Chung
		** Reason:	Wireless 2.4G ??advanced：Do not check the value of Beacon interval, DTIM, Fragment threshold and RTS threshold.
		** Note:	TEW-810DR pre-test no.59
		**/
		...
		obj.set_ccp_act('set');
		obj.add_param_event('adv_wlan0_perform');
		obj.set_param_next_page('wireless_advanced.asp');
		...
		for(var v in arrIns)
		{
			...
			obj.add_param_arg('wlan0_beacon_interval',$('#beacon').val());
			obj.add_param_arg('wlan0_dtim',$('#dtim').val());
			obj.add_param_arg('wlan0_fragmentation',$('#fragment').val());
			obj.add_param_arg('wlan0_rts_threshold',$('#rts').val());
			obj.add_param_arg('wlan0_txpower',$('#tx_power').val());
			obj.add_param_arg('wlan0_short_preamble',$('input[name=short_preamble]:checked').val());
			obj.add_param_arg('wlan0_burst_enable',$('input[name=n_tx_burst]:checked').val());
			...
			obj.add_param_arg('wlan0_multicast_stream_enable',$('input[name=m2u_enable]:checked').val());

			//HT Physical Mode
			obj.add_param_arg('wlan0_disablecoext',($('input[name=n_coexistence]:checked').val() == 1) ? 0 : 1);
			obj.add_param_arg('wlan0_short_gi',$('input[name=n_gi]:checked').val());
			...
			obj.add_param_arg('wlanCfg_AMSDU_','1.'+i+'.0.0',$('input[name=n_amsdu]:checked').val());
			obj.add_param_arg('wlanCfg_AutoBlockACK_','1.'+i+'.0.0',$('input[name=n_autoba]:checked').val());
			obj.add_param_arg('wlanCfg_DeclineBA_','1.'+i+'.0.0',$('input[name=n_declineba]:checked').val());
			obj.add_param_arg('wlan0_enable_ampdu',$('input[name=n_ampdu]:checked').val());
			send_txrate_value();
		}
	...
	{
		var dot11_mode_t = lanCfg.standard[0];

		switch (dot11_mode_t) {
			case '11n':
				obj.add_param_arg('wlan0_11n_txrate', $('#wlan0_11n_txrate :selected').val());
				break;				
			case '11bgn':
				obj.add_param_arg('wlan0_11bgn_txrate', $('#wlan0_11bgn_txrate :selected').val());
				break;
		}
	}
	...
	{
		var obj = get_by_id("wlan0_11n_txrate");
		...
		for (var i = 0; i < txrate_11n.length; i++) {
			...
			if ($('input[name=n_gi]:checked').val() == '1')
	...
	function set_11bgn_txrate()
	{
		var obj = get_by_id("wlan0_11bgn_txrate");
...
//		$('#2040coexi').show();
		if (lanCfg.coexi[0] == '1') var chk_coexi = 0;
		if (lanCfg.coexi[0] == '0') var chk_coexi = 1;
...
//		$('#ampdu').show();
		...
		$('input[name=n_ampdu][value='+chk_ampdu+']').attr('checked', true);
	}
	function setValueMCS()
	{
		...
		switch (lanCfg.standard[0])	{
			case '11n': /* n mode */
				$('#wlan0_11n_txrate').val("<!--# echo wlan0_11n_txrate -->");
				break;
			case '11bgn': /* bgn mixed mode */
				$('#wlan0_11bgn_txrate').val("<!--# echo wlan0_11bgn_txrate -->");
				break;
		}
	}
	...
	{
		var val_wmode = lanCfg.standard[0];
	...
	<td>
		...
		<tr>
			...
			<td style="background-image:url('/image/bg_l.gif');background-repeat:repeat-y;vertical-align:top;" width="270">
				...
				<script>document.write(menu.build_structure(1,2,0))</script>
				<p>&nbsp;</p>
				</div>
...
<div id="div_11n" class="box_tn" style="display:none;">
	...
	<table cellspacing="0" cellpadding="0" class="formarea">
		...
		<tr id="guard_interval">
			<td class="CL"><script>show_words('_help_txt155')</script></td>
			<td class="CR">
				<input type="radio" name="n_gi" value="0" onClick="setValueMCS()" /><script>show_words('_long');</script>&nbsp;
				<input type="radio" name="n_gi" value="1" onClick="setValueMCS()" /><script>show_words('KR50');</script>
			</td>
		</tr>
		<!-- 11n txrate -->
		...
		<tr id="ext_channel" style="display: none;">
		...
		<tr id="ampdu">
			...
			<td class="CR">
				<input type="radio" name="n_ampdu" value="1" /><script>show_words('_enable');</script>&nbsp;
				<input type="radio" name="n_ampdu" value="0" /><script>show_words('_disable');</script>
			</td>
		</tr>
	</table>
</div>
