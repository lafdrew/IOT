...
function send_static_request(){
	...
	var ipv6_6rd_dhcp = "<!--# echo ipv6_6rd_dhcp -->";
        ...
        if (!check_lan_setting(temp_ip_obj, temp_mask_obj, temp_gateway_obj, get_words('_WAN'))){
        	return false;
        }		     
	...
	if ((get_by_id("wan_primary_dns").value =="" || get_by_id("wan_primary_dns").value =="0.0.0.0") && 
		...
		alert(get_words('GW_WAN_DNS_SERVERS_INVALID'));
		return false;
	}
	...
	//	}
		...
		obj.add_param_arg('wan_static_ipaddr',get_by_id("wan_static_ipaddr").value);
		obj.add_param_arg('wan_static_netmask',get_by_id("wan_static_netmask").value);
		obj.add_param_arg('wan_static_gateway',get_by_id("wan_static_gateway").value);
		...
		obj.set_ccp_act('set');
	        obj.add_param_event('wan_static');
		obj.set_param_next_page('internet_wan_static.asp');
