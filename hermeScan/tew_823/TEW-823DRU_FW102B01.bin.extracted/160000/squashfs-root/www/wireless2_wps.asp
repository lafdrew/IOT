...
<head>
<title>TRENDNET | modelName | Wireless 5GHz | WPS</title>
...
<script type="text/javascript">
	...
	var security;
	security = "<!--# echo wlan1_vap0_security -->";
	var sec_vap0 = security.split("_");
	security = "<!--# echo wlan1_vap2_security -->";
	var sec_vap2 = security.split("_");
	security = "<!--# echo wlan1_vap3_security -->";
	var sec_vap3 = security.split("_");

	var wlanCfg = {
	  'enable':          ['<!--# echo wlan1_vap0_enable -->', '<!--# echo wlan1_vap2_enable -->', '<!--# echo wlan1_vap3_enable -->'],
	  'standard':        ['<!--# echo wlan1_dot11_mode -->'],
	  'security':        [sec_vap0[0], sec_vap2[0], sec_vap3[0]],
	  'vs':              ['<!--# echo wlan1_vap0_ssid_broadcast -->']
	};
	var wpaCfg = {
	  'wpamode' :        [sec_vap0[1], sec_vap2[1], sec_vap3[1]],
	  'wpacipher':       ['<!--# echo wlan1_vap0_psk_cipher_type -->','<!--# echo wlan1_vap2_psk_cipher_type -->','<!--# echo wlan1_vap3_psk_cipher_type -->'],
	  'wpakey':          ['<!--# echot wlan1_vap0_psk_pass_phrase -->']
	};
	var wpsCfg = {
	  'enable':          '<!--# echo wlan1_wps_enable -->',
	  'state':           '<!--# echo wlan1_wps_configured_mode -->',
	  'locked':          '<!--# echo wlan1_wps_lock -->',
	  'selfpin':         '<!--# echo wps_pin -->'
	};
	var mac_filter_type = ["<!--# echo wlan1_vap0_mac_filter_type -->", "<!--# echo wlan1_vap1_mac_filter_type -->",
	                       "<!--# echo wlan1_vap2_mac_filter_type -->", "<!--# echo wlan1_vap3_mac_filter_type -->"];
	...
	function wps_apply()
	{
		...
		obj.set_ccp_act('set');
		obj.add_param_event('adv_wps1');
		obj.set_param_next_page('wireless_wps.asp');
		obj.add_param_arg('reboot_type', 'wireless');

		//WPS Config
		obj.add_param_arg('wlan1_wps_enable',$('#WPSEnable').val());
		obj.add_param_arg('wlan1_wps_lock',$('#WPSEG').val());
	...
	function sendWPSPIN()
	{
		...
		var ajax_param = {
			...
			async:  false,
			url:    'apply.cgi',
			data:   'action=set_sta_enrollee_pin_5g'+
	...
	function sendWPSPBC()
	{
		...
		var ajax_param = {
			...
			async:  false,
			url:    'apply.cgi',
			data:   'action=virtual_push_button_5g'+
			        "&"+time+"="+time,
...
<div id="radioOnField" style="display:none;">
	...
	<div id="div_wps_status" class="box_tn" style="display:none;">
		...
		<tr class="break_word">
			...
			<td class="CR"><!--# echot wlan1_vap0_ssid --></td>
		</tr>
		<tr>
