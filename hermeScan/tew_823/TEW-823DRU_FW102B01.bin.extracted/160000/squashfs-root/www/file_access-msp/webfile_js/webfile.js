...
function check_special_char(src_str){
	...
	if (result == 1){
		...
		msg_obj.warning_msg('MSG003', lang_obj.display("WS044"));
	}
...
//Support Web File Access New Spec:Verification Token
function generate_token(para, key){
	var random_num = generate_number(32);
	var url = "/dws/api/" + para;
	var hmac_md5_out = hex_hmac_md5(key, url + random_num);

//	alert("url=" + url + ", url length=" + url.length
//		 +"\r\n"+ "random_num=" + random_num + ", random_num length=" + random_num.length
//		 +"\r\n"+ "key=" + key + ", key length=" + key.length
//		 +"\r\n"+ "hmac_md5_out=" + hmac_md5_out + ", hmac_md5_out length=" + hmac_md5_out.length
//		 +"\r\n"+ "token=" + random_num+hmac_md5_out + ", token length=" + (random_num+hmac_md5_out).length);

	return random_num+hmac_md5_out;
