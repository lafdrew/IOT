...
function check_pin(client_pin_number){
	var checksum = 0;
	var last = parseInt(client_pin_number.charAt(7));

	for (var i = 0; i < 7; i++){
		if (i%2 == 0){
			checksum += 3*parseInt(client_pin_number.charAt(i));
		}else{
			checksum += 1*parseInt(client_pin_number.charAt(i));
		}
	}
...
function get_index_str(index){
	...
	switch(index){
		case 0:
			str = lang_obj.display('WEEK001');
			break;
		case 1:
			str = lang_obj.display('WEEK002');
			break;
		case 2:
			str = lang_obj.display('WEEK003');
			break;
		case 3:
			str = lang_obj.display('WEEK004');
			break;	
	}
...
function check_conflict_application(application_list, checking_protocol, checking_port){
	...
	for (var i = 0; i < application_list.length; i++){
		...
		if ((firewall_protocol == checking_protocol) || (firewall_protocol == "Any")  || (firewall_protocol == "ALL") || (checking_protocol == "Both")){
			
			if (check_multi_port_conflict(obj.get("firewall_port"), checking_port)){
				return false;
			}		
		}
	}
...
function check_conflict_storage_settings(storage_settings, checking_port){
	
	if (storage_settings.get("enable_web_file") == "1"){
		if (check_multi_port_conflict(storage_settings.get("remote_web_port"), checking_port)){
			return false;
		}else if (check_multi_port_conflict(storage_settings.get("ssl_remote_web_port"), checking_port)){
			return false;
		}
	}
...
/**
 *	check_slash()  :	check the input data shouldn't include "/"
 *
 *	Parameter(s) :
 *		data		 :	data that you want to check
 *			 
...
function check_locally_administered_mac(mac, splinter){
	var temp_mac = mac.split(splinter);
	var nibble = hex_to_int((temp_mac[0]).charAt(1));	// get the last 4 bits of first byte
	var nibble_bin = nibble.toString(2);	// convert number to bin number
	var index;
	var result = false;
				
	if (nibble_bin.length >= 2){
		index = nibble_bin.length - 2;
		
		if (nibble_bin.charAt(index) == '1'){	// check the secondary bit is locally administered or not
			result = true;
		}
	}
...
function decode_char(encode_str){
	var ENCODING_LEN = 25;
	var find = false;
	var is_encoding = 0;	
	var replace_str= new Array;
	var temp_str="";
	var i, j, k;

	for (i = 0, j = 0; i < encode_str.length; i++, j++){
		if (encode_str[i] != '%'){
			replace_str[j] = encode_str[i];
		}else{
			find = false;
			...
			if (find == false){	// when the encode char is not in the encoding_code list
				replace_str[j] = encode_str[i];
			}
		}			
	}
	...
	if (is_encoding){
		for (i = 0; i < encode_str.length; i++){
			encode_str[i] = replace_str[i];
		}
	}
}
...
function encode_url_char(encode_str){
	var str = "";
	...
	for (var i = 0; i < encode_str.length; i++){
		...
		for (var j = 0; j < url_encoding_char.length; j++){
			if (ch == url_encoding_char[j]){
				find = true;
			}		
		}
...
/*
2012/05/10 Pin Lee
Storage path need to replace '\' to '%5C' and '|' to '%7C'
For old FW will not display error, and new FW will fix special char bug.
*/

function replace_path(src_str){
	var dest_src = "";
...
function check_is_same (database_obj, array_obj){
	...
	if (database_obj.length != array_obj.length){
		...
		if (!result){
			for (var i = 0; i<database_obj.length; i++){
				if (database_obj[i].get("storage_name") == array_obj[i].get("storage_name")
					&& database_obj[i].get("storage_pwd") == array_obj[i].get("storage_pwd")
					&& database_obj[i].get("storage_path") == array_obj[i].get("storage_path")){
...
function check_upload_name(upload_name){
	var file_name;
