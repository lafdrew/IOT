...
//check session timeout on all page
var sessiontime="<!--# echo session_timeout -->";
...
function session_time_out(){
	...
	if(sessiontime<0){
        ...
        t=setTimeout("session_time_out()",1000);
...
function session_time_out_stop(){
...
function change_wan(){
        ...
        switch(value){
                ...
                case 'rus_l2tp' :
                        ...
                        break;
                case 'dslite' :
                        html_file = "internet_wan_3G.asp";
                        break;
                case 'dslite' :
                        html_file = "internet_wan_dslite.asp";
                        break;
                }
        setTimeout(function() {
	...
	for(var i = 1; i < 5; i++){
			...
			if (wep_def_key[i-1].selectedIndex){
		    	...
		    	if (key.length != wep_key_len && key.length != hex_len){
			      	...
			      	if(i == def_key)
			      		get_by_id("wlan0_vap0_wep_display").value = "hex";
		    	}else{
		    		if(i == def_key)
		    			get_by_id("wlan0_vap0_wep_display").value = "ascii";
...
{
	var key;
	var def_key = get_by_id("wep_def_key1").value;
	var wep_def_key = get_by_id("wep_def_key1");
	...
	for(var i = 1; i < 2; i++){
		...
		if (wep_def_key[i-1].selectedIndex){
			...
			if (key.length != wep_key_len && key.length != hex_len){
				...
				if(i == def_key)
					get_by_id("wlan1_vap0_wep_display").value = "hex";
			}else{
				if(i == def_key)
					get_by_id("wlan1_vap0_wep_display").value = "ascii";
...
function check_vap1_key_1(){
	var key;
	var def_key = get_by_id("wep_def_key1").value;
	var wep_def_key = get_by_id("wep1_def_key1");
	var wep_key_len = parseInt(get_by_id("wep1_key_len_1").value);
	var hex_len = wep_key_len * 2;
...
function jq_ajax_post(url, data){
...
{	
	if (percent == null)
	{
		...
		//redirectURL = ".." + location.pathname;
		document.getElementById("mainform").style.display = "none";
		document.getElementById("waitform").innerHTML = "<table bolder=\"0\"><tr><td colspan=\"2\">"+get_words('_rebooting_plz_wait')+"</td></tr><tr><td width=\"95%\"><table align=\"left\" bgcolor=\"#FFFFFF\" cellpadding=\"0\" cellspacing=\"0\" bordercolor=\"#000000\" style=\"border-style: solid; border-width: 1px\"><tr><td width=545 align=\"left\"><table id=\"progress\" bgcolor=\"blue\" height=\"25\"><tr><td></td></tr></table></td></tr></table></td><td width=\"95%\"><font color=white><span id=\"progressValue\">&nbsp;</span></font></td></tr></table>";
	}
	percent = (totalWaitTime*1000-(endClock-new Date().getTime()))/(totalWaitTime*10);
	if (percent >= 100) {
		//if(redirectURL == null || redirectURL == "")
		//	redirectURL = "/";
		top.location.href = "http://"+redirectURL;
		return;
	}
	document.getElementById("progress").style.width = Math.round(percent) + "%";
	document.getElementById("progressValue").innerHTML = Math.round(percent) + "%";
  ...
  var wlan1_sec = isExist_var("wlan1_security") ? getVal("wlan1_security") : "-1";
	...
	if (wlan0_sec != "-1") {
		if (wlan0_sec.match(/wep/g) && wlan0_gsec.match(/wep/g)) {
			alert(LangMap.which_lang['WIFI_WEP_CONFLICT']);
			return false;
		}
	}
	...
	if ("<!--# echo wps_enable -->" == "1") {
		...
		if (w_en) {
			if (get_by_id("wps_configured_mode").value !== "1" && wlan0_sec.match(/disable/g)) {
 	               		alert(LangMap.which_lang['WPS_WARR_OPEN_CONFIG']);
			}

			if (wlan0_sec.match(/wep/g) && !confirm(LangMap.which_lang['WPS_WARR_WEP'])) {
				return false;
                	}

			if (wlan0_cipher == "tkip" && wlan0_sec.match(/wpa/g) && !confirm(LangMap.which_lang['WPS_WARR_TKIP'])) {
				return false;
			}

			if ((wlan0_sec.match(/disable/g) == null) && (wlan0_cipher.match(/tkip/g) == null) &&
			      (wlan0_sec.match(/wpa2/g) == null) && (wlan0_sec.match(/wep/g) == null) && 
			      (wlan0_sec.match(/eap/g) == null ) && !confirm(LangMap.which_lang['WPS_WARR_WPAPSKONLY'])) {
				return false;
			}

			if (wlan0_sec.match(/eap/g) && !confirm(LangMap.which_lang['WPS_WARR_WPAEAP'])) {
				return false;
			}

			vflag = getNames("wlan0_vap0_ssid_broadcast");
		
		if (vflag && !confirm(LangMap.which_lang['WPS_WARR_SSID_BROADCAST']))
					return false;
		}
		...
		if (w_en_1) {
			...
			if (wlan1_sec.match(/wep/g) && !confirm(LangMap.which_lang['WPS_WARR_WEP'])) {
				return false;
                	}

			if (wlan1_cipher == "tkip" && wlan1_sec.match(/wpa/g) && !confirm(LangMap.which_lang['WPS_WARR_TKIP'])) {
				return false;
			}

			if ((wlan1_sec.match(/disable/g) == null) && (wlan1_cipher.match(/tkip/g) == null) &&
			      (wlan1_sec.match(/wpa2/g) == null) && (wlan1_sec.match(/wep/g) == null) && 
			      (wlan1_sec.match(/eap/g) == null ) && !confirm(LangMap.which_lang['WPS_WARR_WPAPSKONLY'])) {
				return false;
			}

			if (wlan1_sec.match(/eap/g) && !confirm(LangMap.which_lang['WPS_WARR_WPAEAP'])) {
				return false;
			}

			vflag1 = getNames("wlan1_vap0_ssid_broadcast");
		
			if (vflag1 && !confirm(LangMap.which_lang['WPS_WARR_SSID_BROADCAST']))
					return false;
		}
	}
	return true;
...
function WPS()
{
	...
	if (wlan0_en != -1 && wlan0_en == "1")
		wps_behavior(wlan0_sec, wlan0_cipher, getNames("wlan0_vap0_ssid_broadcast"));

	if (wlan1_en != -1 && wlan1_en == "1")
		wps_behavior(wlan1_sec, wlan1_cipher, getNames("wlan1_vap0_ssid_broadcast"));
}       
