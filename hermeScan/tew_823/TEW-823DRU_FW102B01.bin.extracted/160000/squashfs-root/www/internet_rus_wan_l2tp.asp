...
<script type="text/javascript">
    ...
    function onPageLoad()
    {
		...
		set_selectIndex("<!--# echo wan_l2tp_dynamic -->", get_by_id("wan_l2tp_dynamic"));
		set_selectIndex("<!--# echo wan_l2tp_connect_mode -->", get_by_id("wan_l2tp_connect_mode"));
		//show_AdvDns();
    ...
    {   
        var conn_mode = get_by_id("wan_l2tp_connect_mode").selectedIndex;
    ...
    {
        var fixIP = get_by_id("wan_l2tp_dynamic").selectedIndex;
        if (fixIP=="1"){ 
        get_by_id("wan_l2tp_ipaddr").disabled = true;
        get_by_id("wan_l2tp_netmask").disabled = true;
        get_by_id("wan_l2tp_gateway").disabled = true;
        get_by_id("DNSServer").disabled = true;
        
        get_by_id("rl2tpIp").style.display = "none";
        get_by_id("rl2tpNetmask").style.display = "none";
        get_by_id("rl2tpGateway").style.display = "none";
        get_by_id("DNSServer").style.display = "none";
        }else{
        get_by_id("wan_l2tp_ipaddr").disabled = false;
        get_by_id("wan_l2tp_netmask").disabled = false;
        get_by_id("wan_l2tp_gateway").disabled = false;
        get_by_id("DNSServer").disabled = false;
        
        get_by_id("rl2tpIp").style.display = "";
        get_by_id("rl2tpNetmask").style.display = "";
        get_by_id("rl2tpGateway").style.display = "";
	...
	//form1 always modified? why?
		...
		var wan_type = $('#wan_l2tp_dynamic').val();
		...
		var server_ip = $('#wan_l2tp_server_ip').val();
		...
		**/
		...
		connect_mode = $("#wan_l2tp_connect_mode").val();
...
*/
	...
	//	}
		...
		if(submit_button_flag == 0){
                        ...
                        obj.add_param_arg('wan_l2tp_ipaddr',get_by_id("wan_l2tp_ipaddr").value);
                        obj.add_param_arg('wan_l2tp_netmask',get_by_id("wan_l2tp_netmask").value);
                        obj.add_param_arg('wan_l2tp_gateway',get_by_id("wan_l2tp_gateway").value);
                        obj.add_param_arg('wan_l2tp_server_ip',get_by_id("wan_l2tp_server_ip").value);
                        obj.add_param_arg('wan_l2tp_username',get_by_id("wan_l2tp_username").value);
                        ...
                        obj.add_param_arg('wan_l2tp_dynamic',get_by_id("wan_l2tp_dynamic").value);
	                ...
	                if(get_by_id("wan_l2tp_dynamic").value == 1)
                        ...
                        obj.add_param_arg('wan_l2tp_russia_enable',1);
                                               
                        obj.set_param_url('apply.cgi');
                        obj.set_ccp_act('set');
                        obj.add_param_event('wan_rus_l2tp');
                        obj.set_param_next_page('internet_rus_wan_l2tp.asp');
                        if(get_by_id("l2tppwd1").value !="WDB8WvbXdH"){
                                obj.add_param_arg('wan_l2tp_password',get_by_id("l2tppwd1").value);
                        }
	...
	<td>
		...
		<tr>
			...
			<td style="background-image:url('/image/bg_r.gif');background-repeat:repeat-y;vertical-align:top;" width="680">
				...
				<input type="hidden" id="wan_l2tp_russia_enable" name="wan_l2tp_russia_enable" value="1">
	...
	<table cellspacing="0" cellpadding="0" class="formarea">
		<tr>
			...
			<td class="CR">
				...
				<option value="rus_l2tp" selected>Russia L2TP</option>
...
<div id="l2tp" class="box_tn" style="visibility: visible; display: block;">
	...
	<table id="l2tp" cellspacing="0" cellpadding="0" class="formarea">
		...
		<tr>
			...
			<td class="CR">
				<input type="text" id="wan_l2tp_server_ip" name="wan_l2tp_server_ip" size="32" maxlength="64" value="<!--# echo wan_l2tp_server_ip -->" />
			</td>
		</tr>
		<tr>
			<td class="CL"><script>show_words('_username')</script></td>
			<td class="CR">
				<input type="text" id="wan_l2tp_username" name="wan_l2tp_username" size="32" maxlength="64" value="<!--# echo wan_l2tp_username -->" />
			</td>
		</tr>
		<tr>
			<td class="CL"><script>show_words('_password')</script></td>
			<td class="CR">
				<input type="password" id="l2tppwd1" name="l2tppwd1" size="32" maxlength="64" value="WDB8WvbXdH" />
			</td>
		</tr>
<!--		<tr>
...
-->
		<tr>
			...
			<td class="CR">
				<select id="wan_l2tp_dynamic" name="wan_l2tp_dynamic" size="1" onChange="disable_l2tp_ip()">
		...
		<tr id="rl2tpIp">
			...
			<td class="CR">
				<input type="text" id="wan_l2tp_ipaddr" name="wan_l2tp_ipaddr" size="15" maxlength="15" value="<!--# echo wan_l2tp_ipaddr -->" />
			</td>
		</tr>
		<tr id="rl2tpNetmask">
			<td class="CL"><script>show_words('_subnet')</script></td>
			<td class="CR">
				<input type="text" id="wan_l2tp_netmask" name="wan_l2tp_netmask" size="15" maxlength="15" value="<!--# echo wan_l2tp_netmask -->" />
			</td>
		</tr>
		<tr id="rl2tpGateway">
			<td class="CL"><script>show_words('_defgw')</script></td>
			<td class="CR">
				<input name="wan_l2tp_gateway" type="text" id="wan_l2tp_gateway" size="15" maxlength="15" value="<!--# echo wan_l2tp_gateway -->" />
			</td>
		</tr>
		<tr>
			<td class="CL" rowspan="2" id="wL2tpOPMode"><script>show_words('_opeartion_mode');</script></td>
			<td class="CR">
				<select id="wan_l2tp_connect_mode" name="wan_l2tp_connect_mode" size="1" onChange="check_connectmode()">
		...
		<tr>
			<td colspan="2" class="CELL">
				...
				<input type="text" id="wan_l2tp_max_idle_time" name="wan_l2tp_max_idle_time" maxlength="3" size="2" value="<!--# echo wan_l2tp_max_idle_time -->">
...
<div class="box_tn" id="mtu_field" style="display: block;">
	...
	<table cellspacing="0" cellpadding="0" class="formarea">
		...
		<tr>
			...
			<td class="CR">
				<input type="text" id="wan_l2tp_mtu" name="wan_l2tp_mtu" maxlength="4" value="<!--# echo wan_l2tp_mtu -->" />&nbsp;<script>document.write(get_words('_mtu_default_byte').replace('%s', 1400));</script>
			</td>
		</tr>
