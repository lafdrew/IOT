...
{
	if(get_checked_value(get_by_id('hnatEnable')) == "1")
	{
...
/*
** switch between input[type=password] and input[type=text]
** tested under chrome,safari,fx,IE6,IE7,IE8,IE10
** checked - password, unckecked - text
** @author:	Moa Chung
** @date:	2013-09-12
** @param:	that - the checkbox DOM
** 			id - the id of password box
** 			revert - set checked type, true=password,false=text(default:false)
*/
function switch_input_type(that, id, revert)
{
	var rAttr = ['size','maxlength','default','class','style', 'name', 'onChange'];//reserved attribute 	**case sensitive
	var oNode = $('#'+id);
	var oAttr = oNode[0].attributes;
	var nNode = $('<'+oNode[0].tagName+'>').attr({
			id:id+'_',
			type:(($(that).is(':checked') ^ revert)?'password':'text'),
			value:oNode.val()
		});
	for(var i=0;i<rAttr.length;++i)
	{
		if(oAttr[rAttr[i]])
			nNode.attr(rAttr[i], oAttr[rAttr[i]].value);
	}
	oNode.after(nNode);
	oNode.remove();
	nNode.attr('id',id);
