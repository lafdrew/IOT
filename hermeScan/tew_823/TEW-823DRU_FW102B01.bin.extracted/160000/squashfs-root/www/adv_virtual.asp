...
//	menu.setSupportUSB(dev_info.KCode_USB);
	var array_enable = [], array_name = [], array_Protcol_num = [],
		array_PubPort = [], array_PriPort = [], array_intnlip = [], 
		array_Schdule = [], array_Policy = [];
...
//		var kcode_ftp_wan		= main.config_val("igdFTPServer_AccessWanEnable_");
	...
	array_proto_name[6] = get_words('_TCP');
	array_proto_name[17] = get_words('_UDP');
	array_proto_name[256] = get_words('_both');
	...
	function onPageLoad()
	{
		...
		for (j=0; j<rule_max_num; j++) {
			...
			array_name[j] = temp[1];
			array_Protcol_num[j] = temp[2];
			array_PubPort[j] = temp[3];
			array_PriPort[j] = temp[4];
			array_intnlip[j] = temp[5];
			array_Schdule[j] = temp[6];
			array_Policy[j] = temp[7];
		}
		...
		set_virtualserver();
		paintTable();
		do_select('protocol_select', 'val', array_Protcol_num);
		protocol_change();
		$('#protocol').hide();
	}
	
	function Data(enable, name, protcol_num, pubPort, priPort, ip_addr, schdule, policy, onList)
	{
		this.Enable = enable;
		this.Name = name;
		...
		for (var i = 0; i < rule_max_num; i++) {
			if(array_name[i] != "" && array_name[i])
			{
				...
				DataArray[DataArray.length++] =
					new Data(array_enable[i], array_name[i], array_Protcol_num[i], array_PubPort[i], array_PriPort[i], array_intnlip[i], array_Schdule[i], array_Policy[i], i);
			}
		}
	...
	function update_DataArray()
	{
		...
		if(insert){
			DataArray[index] = new Data(is_chk, $('#name').val(), $('#protocol_select').val(), $('#public_portS').val(), $('#private_portS').val(), $('#ip').val(), $('#schedule').val(), $('#inbound_filter').val(), index+1);			
		}else if (index != -1){	
			DataArray[index].Enable =is_chk;
			DataArray[index].Name = $('#name').val();
			DataArray[index].Protcol_num = $('#protocol_select').val();
	...
	function AddVStoDatamodel()
	{
		...
		obj.set_ccp_act('set');
		obj.add_param_event('adv_virtual');
		...
		for(var i=0; i<DataArray.length; i++)
		{
			...
			value = DataArray[i].Enable + "/" + DataArray[i].Name + "/" + DataArray[i].Protcol_num + "/" +
				DataArray[i].PubPort + "/" + DataArray[i].PriPort + "/" + DataArray[i].Ip_addr + "/" +
	...
	function send_request()
	{
		...
		try
		{
			if(remote_port_enable == '1')
			{
				...
				var tcp_ports_https = remote_config[2];
...
//			}
			...
			if(protocol != -1)//do check if not choose specific protocol(Other)
			{
				...
				if (!check_port($('#public_portS').val())) {
					...
					return false;
				}else if (reg.test($('#public_portS').val()) == true){
					alert(get_words('virtual_pub_port_err'));
					return false;
				}else if (reg.test($('#private_portS').val()) == true){
					alert(get_words('virtual_pri_port_err'));
					return false;
				}
			}
	...
	{	
		...
		for(var i = 0; i < DataArray.length; i++)
		{
			...
			contain += '<tr align="center">'+
					...
					'</td><td align="center" class="CELL break_word">' + array_proto_name[DataArray[i].Protcol_num] + ','
															 + DataArray[i].PubPort + '/' + DataArray[i].PriPort +
					'</td><td align="center" class="CELL break_word">'+ DataArray[i].Policy +
					'</td><td align="center" class="CELL break_word">'+ DataArray[i].Schdule +
...
<div class="box_tn">
	...
	<tr>
		...
		<td class="CR">
			<input type="checkbox" id="enable" name="enable" value="1" onchange="changeSense()" />
		</td>
	</tr>
	<tr>
	...
	<tr>
		...
		<td class="CR">
			<input type="text" id="ip" name="ip" size="16" maxlength="15" value="" onchange="changeSense()" />
		</td>
	</tr>
	<tr>
	...
	<tr>
		...
		<td class="CR"><input type="text" id="public_portS" name="public_portS" size="16" maxlength="5" value="" onchange="changeSense()" /></td>
	</tr>
	<tr>
		<td class="CL"><script>show_words('av_PriP');</script></td>
		<td class="CR"><input type="text" id="private_portS" name="private_portS" size="16" maxlength="5" value="" onchange="changeSense()" /></td>
	</tr>
	<tr>
		<td class="CL">
