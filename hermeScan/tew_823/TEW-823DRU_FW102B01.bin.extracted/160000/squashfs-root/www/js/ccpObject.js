...
function ccpObject(ccp,act) {
	...
	this.get_router_info = function() {
		...
		var info = {
			...
			'domain_a':		this.config_val('RF_Domain_A'),//DIR not used
...
*/
	...
	*/
	...
	this.set_param_next_page = function(nextPage) {
		this.set_param_option('html_response_return_page',tryEncodeURI(nextPage));
	};
	...
	this.add_param_arg = function(oid, val) {
		param_arg.push({'oid':oid,'val':tryEncodeURI(val)});
	};
	...
	*/
	...
	var parse_event_to_param_arg = function(){
		for(var i=0;i<param_arg_event.length;i++)
			param.arg += ((i==0?'&action=':'&action'+(i+1)+'=')+param_arg_event[i]);
	}
	var parse_param_arg_by_ccp_act = function(){
		if(ccp_act=='set')
		{
			...
			{
				param.arg += '&'+param_arg[i].oid+'='+param_arg[i].val;
			}
		}
		else
		{
