...
list_deny()
{
	...
	while [ $mac_n -le 23 ]; do
		...
		if [ $en -eq 1 ]; then
			if [ $debug -eq 1 ]; then
				echo "# mcastd acl add -n $acl_n -s $mac -j 7 -p $mac_port"
			fi
			mcastd acl add -n $acl_n -s $mac -j 7 -p $mac_port
			acl_n=$(($acl_n+1))
		fi
		mac_n=$(($mac_n+1))
	done
	...
	while [ $mac_n -le 23 ]; do
		...
		if [ $en -eq 1 ]; then
			...
			if [ $mac_n != $last_mac_n ]; then
				mcastd acl add -n $acl_n -s $mac -j 0 -p $mac_port
				acl_n=$(($acl_n+1))
			else
				mcastd acl add -n $acl_n -i -s $mac -j 0 -p $mac_port
				acl_n=$(($acl_n+1))
				mcastd acl add -n $acl_n -i -s $mac -j 7 -p $mac_port
			fi
		fi
		mac_n=$(($mac_n+1))
	done
